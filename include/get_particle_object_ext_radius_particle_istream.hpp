...
class get_particle_object_ext_radius_particle_istream : public frantic::particles::streams::delegated_particle_istream {
    ...
    void populate_particle_radius( char* p ) {
        ...
        if( !m_idAccessor.is_valid() ) {
            throw std::runtime_error( "get_particle_object_ext_radius_particle_istream.populate_particle_radius Error: "
                                      "the particles do not have the required \'" +
                                      frantic::strings::to_string( m_idChannelName ) + "\' ID channel." );
        }

        const int id = m_idAccessor( p );
        const int index = m_particles->GetParticleIndex( id );
        const float radius = 0.5f * m_particles->GetParticleScaleByIndex( index );

        m_radiusAccessor.set( p, radius );
    }

  public:
    get_particle_object_ext_radius_particle_istream(
        ...
        if( !particles ) {
            throw std::runtime_error( "get_particle_object_ext_radius_particle_istream Error: particles is NULL" );
        }
    ...
    virtual ~get_particle_object_ext_radius_particle_istream() {}
