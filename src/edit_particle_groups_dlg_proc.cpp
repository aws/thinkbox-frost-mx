  ...
  public:
    static INT_PTR CALLBACK DlgProc( __in HWND hwndDlg, __in UINT uMsg, __in WPARAM wParam, __in LPARAM lParam ) {
        try {
            ...
            switch( uMsg ) {
            case WM_INITDIALOG: {
                GetFrostGuiResources()->apply_custbutton_right_arrow_icon( GetDlgItem( hwndDlg, IDC_INCLUDE ) );
                GetFrostGuiResources()->apply_custbutton_left_arrow_icon( GetDlgItem( hwndDlg, IDC_EXCLUDE ) );
