...
NO_LICENSE_ALLOWLIST: set[str] = {
    ...
    'maxsdk',
    # The V-Ray SDK is not open source and we do not distribute it
    'vraysdk',
...
class FrostVRYConan(ConanFile):
    ...
    def build(self) -> None:
        version_gen.write_version_resource(self.version, os.path.join(self.source_folder, 'FrostVersion.rc'))
        version_gen.write_version_header(self.version, os.path.join(self.source_folder, 'FrostVersion.h'))
    ...
    def export_sources(self) -> None:
        shutil.copy('../include/FrostInterface.hpp', 'include')
        self.copy('**.h', src='', dst='')
